...
//<---------------- Pay tip ----------->>>>
    ...
    $(document).on('click', '#donateBtn', function (event) {
        ...
        if (isValid) {
            ...
            var form = $(this).attr('data-form');
            element.attr({'disabled': 'true'});
            var payment = $('input[name=payment_gateway_donate]:checked').val();
            element.find('i').addClass('spinner-border spinner-border-sm align-middle mr-1');
            (function () {
                $('#formSendDonate').ajaxForm({
                    dataType: 'json',
                    success: function (result) {
                        // Wallet
                        if (result.success == true && payment == 'wallet' || result.success && result.instantPayment) {
                            swal({
                                title: thanks,
                                text: donation_sent_success,
                                type: "success",
                                confirmButtonText: ok
                            });
                            $('#donateForm').modal('hide');
                            $('.InputElement').val('');
                            $('#formSendDonate').trigger("reset");
...
//============ End Payment =================//
    ...
    $('#donateForm').on('hidden.bs.modal', function (e) {
        var modal = $(this);
        $('#errorDonate').hide();
        $('#formSendDonate').trigger("reset");
        $('#card-errors').addClass('display-none');
    ...
    $('#donateForm').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget) // Button that triggered the modal
        var coverUser = button.data('cover') // Extract info from data-* attributes
        var avatarUser = button.data('avatar') // Extract info from data-* attributes
        var fullNameUser = button.data('name') // Extract info from data-* attributes
        ...
        $('#donateForm').find('input[name="id"]').val(id);
