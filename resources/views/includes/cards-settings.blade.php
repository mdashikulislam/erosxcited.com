          ...
          @if ($settings->referral_system == 'on' || auth()->user()->referrals()->count() != 0)
			  ...
			  @endif
                ...
                <a href="{{route('donation.index')}}"
                   class="list-group-item list-group-item-action d-flex justify-content-between @if (request()->is('donation/*')) active @endif">
                    <div>
                        <i class=" bi-coin mr-2"></i>
                        <span>{{ trans('general.donation_goals')}}</span>
                    </div>
                    <div>
                        <i class="feather icon-chevron-right"></i>
                    </div>
