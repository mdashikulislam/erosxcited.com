@if(auth()->check())
<div class="modal fade" id="donateForm" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
                ...
                <div class="card bg-white shadow border-0">
                    ...
                    <div class="card-body px-lg-5 py-lg-5 position-relative">
                        <div class="text-muted text-center mb-3 position-relative modal-offset">
                            ...
                            <h6>
                                {{trans('general.send_donation')}} <span class="userNameTip"></span>
                            </h6>
                        </div>
                        <form method="post" action="{{url('send/donate')}}" id="formSendDonate">
                            ...
                            <input type="number" autocomplete="off" id="onlyNumber2" class="form-control mb-3 tipAmount" required name="amount" placeholder="{{trans('general.donate_amount')}}">
                            @csrf
                            @if (! request()->route()->named('live'))
                                @foreach ($paymentGatewaysSubscription as $payment)
                                    ...
                                    <div class="custom-control custom-radio mb-3">
                                        <input name="payment_gateway_donate" required value="{{$payment->name}}" id="donate_radio{{$payment->name}}" @if ($allPayments->count() == 1 && Helper::userWallet('balance') == 0) checked @endif class="custom-control-input" type="radio">
                                        <label class="custom-control-label" for="donate_radio{{$payment->name}}">
                                            <span><strong>{!!$paymentName!!}</strong></span>
                                        </label>
                                    </div>
                                    @if ($payment->name == 'Stripe')
                                        <div id="stripeContainerDonate" class="@if ($allPayments->count() != 1) display-none @endif">
                            ...
                            @if ($settings->disable_wallet == 'on' && Helper::userWallet('balance') != 0 || $settings->disable_wallet == 'off')
                                <div class="custom-control custom-radio mb-3">
                                    <input name="payment_gateway_donate" required @if (Helper::userWallet('balance') == 0) disabled @endif value="wallet" id="donate_radio0" class="custom-control-input" type="radio">
                                    <label class="custom-control-label" for="donate_radio0">
									<span>
										<strong>
                            ...
                            <div class="alert alert-danger display-none" id="errorDonate">
                                <ul class="list-unstyled m-0" id="showErrorsDonate"></ul>
                            </div>

                            <div class="text-center">
                                <button type="button" class="btn e-none mt-4" data-dismiss="modal">{{trans('admin.cancel')}}</button>
                                <button type="submit" id="donateBtn" class="btn btn-primary mt-4"><i></i> {{trans('auth.send')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
