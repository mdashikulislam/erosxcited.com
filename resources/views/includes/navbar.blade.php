			...
			@endif
				...
				<!-- =========== End Menu Mobile ============-->
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2" href="{{url('/')}}" title="{{trans('admin.home')}}">
							<i class="bi bi-house icon-navbar"></i>
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2" href="{{url('creators')}}" title="{{trans('general.explore_creators')}}">
    							<i class="bi bi-compass icon-navbar"></i>
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2" href="{{url('shop')}}" title="{{trans('general.shop')}}">
							<i class="bi bi-handbag icon-navbar"></i>
				...
				<li class="nav-item dropdown d-lg-block d-none">
					...
					<div class="dropdown-menu mb-1 dropdown-menu-right dd-menu-user" aria-labelledby="nav-inner-success_dropdown_1">
						@if(auth()->user()->role == 'admin')
								<a class="dropdown-item dropdown-navbar" href="{{url('panel/admin')}}"><i class="bi bi-sliders mr-2"></i> {{trans('admin.admin')}}</a>
								<div class="dropdown-divider"></div>
						@endif

						@if (auth()->user()->verified_id == 'yes' || $settings->referral_system == 'on' || auth()->user()->balance != 0.00)
						<span class="dropdown-item dropdown-navbar balance">
							<i class="bi bi-cash-coin mr-2"></i> {{trans('general.balance')}}: {{Helper::amountFormatDecimal(auth()->user()->balance)}}
						</span>
					@endif
					...
					@if ($settings->disable_wallet == 'on' && auth()->user()->wallet != 0.00 || $settings->disable_wallet == 'off')
						@if ($settings->disable_wallet == 'off')
							<a class="dropdown-item dropdown-navbar" href="{{url('my/wallet')}}">
								<i class="bi bi-wallet mr-2"></i> {{trans('general.wallet')}}:
					...
					@endif
						...
						@if (auth()->user()->verified_id == 'yes')
						<a class="dropdown-item dropdown-navbar" href="{{url('dashboard')}}"><i class="bi bi-sliders mr-2"></i> {{trans('admin.dashboard')}}</a>
						<a class="dropdown-item dropdown-navbar" href="{{url('my/posts')}}"><i class="bi bi-card-checklist mr-2"></i> {{trans('general.my_posts')}}</a>
					@endif
					...
					@endif
						<a class="dropdown-item dropdown-navbar" href="{{url('my/subscriptions')}}"><i class="bi bi-check mr-2"></i> {{trans('users.my_subscriptions')}}</a>
				...
				<li class="nav-item">
    <a class="nav-link  btn-arrow-sm btn btn-main btn-primary pr-3 pl-3" href="{{url('settings/page')}}">
        {{ auth()->user()->verified_id == 'yes' ? trans('general.edit_my_page') : trans('users.edit_profile') }}
                <i class="fas fa-pencil-alt mr-1"></i> <!-- Icono de lápiz -->
