                        ...
                        <ul class="nav nav-profile justify-content-center nav-fill">
                            ...
                            @if ($settings->shop || ! $settings->shop && $userProducts->count() != 0)
                                <li class="nav-link @if (request()->path() == $user->username.'/shop')active @endif navbar-user-mobile">
                                    ...
                                    <a href="{{request()->path() == $user->username.'/shop' ? 'javascript:;' : url($user->username, 'shop')}}" title="{{trans('general.shop')}}"><i class="bi bi-handbag"></i> <span class="d-lg-inline-block d-none">{{trans('general.shop')}}</span></a>
                                </li>
                            @endif
        ...
        @if ($user->verified_id == 'yes' && request('media') != 'shop')
            <div class="container py-4 pb-5">
                <div class="row">
                    ...
                    <div class="col-lg-8 wrap-post order-2 order-lg-1">
              ...
              <span class="btn-block mb-3">
                <i class="fa fa-lock ico-no-result"></i>
                    ...
                    <div class="col-lg-4 mb-3 order-1 order-lg-2">
                    ...
                    {!! Helper::checkText($user->story)  !!}
                            ...
                            @php
                                $donations = \App\Models\Donation::activeItem()
                                        ->with('users')
                                        ->withSum('activeHistories','gross_amount')
                                ...
                                $donations = $donations->map(function ($donation) {
                                    ...
                                    $donation->collected_percentage = round($percentage, 1);
                                    return $donation;
                                });
                            @endphp
                            ...
                            @forelse($donations as $donation)
                                <div class="card mb-3 rounded-large shadow-large">
                                    <div class="card-body">
                                        ...
                                        <div>
                                            <img style="width: 100%;height: auto;border-radius: 12px" src="{{asset('/public/uploads/donation/'.$donation->preview)}}" alt="">
                                        </div>
                                        <p style="margin-top: 20px;margin-bottom:0">({{$donation->collected_percentage}}% {{__('general.of')}} {{Helper::amountFormatDecimal($donation->amount)}})</p>
                                        <div class="progress" >
                                            <div class="progress-bar" role="progressbar" style="width: {{$donation->collected_percentage}}%;" aria-valuenow="{{$donation->collected_percentage}}" aria-valuemin="0" aria-valuemax="100">
                                                <span></span>
                                            </div>
                                        </div>
                                        
                                        <button  
                                            @guest data-toggle="modal" data-target="#loginFormModal" @else data-toggle="modal" data-target="#donateForm" @endguest
                                            ...
                                            data-userid="{{$user->id}}"
                                            type="button" {{ $donation->user_id == auth()->id() ? 'disabled' : '' }} 
                                            data-id="{{ $donation->id }}"  
                                            class="btn btn-primary btn-block mt-3 donate-btn">
                                            <i class="bi bi-coin"></i> {{ __('general.donate_now') }}
                                        </button>
