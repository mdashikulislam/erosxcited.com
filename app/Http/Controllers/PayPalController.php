                ...
                if ($data) {
                    switch ($data['type']) {
                        ...
                        case 'tip':
                            ...
                            break;
                        case 'donation':
                            $donation = Donation::with('users')->where('id', $data['id'])->first();
                            $sender = User::find($data['sender']);
                            $earnings = $this->earningsAdminUser($donation->users->custom_fee, $data['amount'], $payment->fee, $payment->fee_cents);
                            $verifiedTxnId = Transactions::where('txn_id', $txnId)->first();
                            if (!isset($verifiedTxnId)) {
                                $this->transaction(
                                    $txnId,
                                ...
                                DonationHistory::create([
                                    ...
                                    'donor_id' => $data['sender'],
                                    'gross_amount' => $data['amount'],
                                    'net_amount' => $earnings['user'],
                                    'status' => '1',
                                ]);
                                return redirect(url('paypal/donate/success', $donation->users->username));
                            }
                            break;
                    }// Switch case
                }// data
